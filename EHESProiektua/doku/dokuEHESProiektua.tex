...
\begin{enumerate}
	...
	\item {\sc Ondorioak / Etorkizunerako lana \dotfill 5}
	\item {\sc Bibliografia \dotfill 5}
	\item {\sc Balorazio subjetiboa \dotfill 6}
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item Aurreko fitxategia arreprozezatu ondoren, arff fitxategi berria sortu nahi den helbidea. Adibidez {\blue devaurre.arff} fitxategia sortzen genuen, baina edozein izen har dezake.
			\item Entrenamendu {\bf .arff} fitxategia. Kasu honetan, {\blue train.p3.resampled.obfuscated.arff} fitxategia.
			\item Hirugarren parametroan adierazi den fitxategia arreprozezatu ondoren, arff fitxategi berria sortu nahi den helbidea. Adibidez {\blue trainaurre.arff} fitxategia sortzen genuen, baina edozein izen har dezake.
			\item Azkenik, beharrezko den parametro bat sar daiteke. Parametro honek, {\bf 1-100} arteko zenbaki bat izan behar da eta fitxategien instantzien \% portsentai jakin batean lan egin nahi den adierazten da. Adibidez fitxategi osoaren instantzien {\bf \%80}-rekin lan egin nahi bada parametro honetan, 80 zenbakia sartzen da.
		\end{enumerate}
		...
		\item Modeloa egitea:\\
			...
			\begin{enumerate}
				\item Aurreprozezatutako development {\bf .arff} fitxategia. Kasu honetan, {\blue devaurre.arff} fitxategia.
				\item Aurreprozazatutako entrenamendu {\bf .arff} fitxategia. Kasu honetan, {\blue trainaurre.arff} fitxategia.
			\end{enumerate}
			Beraz, exekutatzeko, konsolatik honako komandoa egikaritu beharko zen:\\
		...
		\item Sailkatzailea:\\
			...
			\begin{enumerate}
				\item Aurreko exkutagarrian ({\red GetModel.jar}) sorturiko {\bf .model} fitxategi bitarra izango da. Kasu honetan, {\blue BaselineModel.model} edo {\blue MultilayerPerceptronModel.model} fitxategi bitarrak.
				\item .
			\end{enumerate}
			Beraz, exekutatzeko, konsolatik honako komandoa egikaritu beharko zen:\\
				\fbox{{\scriptsize > java -jar Model.jar devaurre.arff trainaurre.arff}}\\\\
			Exekutatu ondoren, programa honek, `modeloak' katalogoa sortzen du eta bere barnean, {\blue BaselineModel.model} eta {\blue MultilayerPerceptronModel.model} modelo fitxategi bitarrak sortzen ditu.
	\end{enumerate}
\end{enumerate}{
